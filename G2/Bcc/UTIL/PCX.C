...
asm {
	mov ah,0fh      // busca modo de v¡deo
	int 10h
	mov ah,0
	mov mvid,ax
}
...
/* mira si es modo 320x200x256 */
if(modo_video()!=0x13) return(E_PCX_MODO);
...
if((y+altura)>200) altura=200-y;
if((x+anchura)>320) anchura=320-x;
...
/* mira si es modo 640x480x16 */
if(modo_video()!=0x12) return(E_PCX_MODO);
...
/* si el gr fico a dibujar no cabe en la pantalla */
/* ajusta las dimensiones para que quepa */
if((y+altura)>480) altura=480-y;
if((x+anchura)>640) anchura=640-x;
