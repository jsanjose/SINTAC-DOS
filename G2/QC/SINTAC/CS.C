...
STC_ELEM_INPUT inpfent={
	"^Fichero entrada"
};
STC_ELEM_INPUT inpfsal={
	"Fichero ^salida"
};
STC_ELEM_BOTON bcomp={
	" ^Compilar"
};
...
STC_ELEM_LISTA lsterr={
	" ^Lista de errores ", C_LSTNORMAL, C_LSTSINORDEN
};
...
/* colores */
BYTE color_cs=COLOR_CS;
BYTE color_css1=COLOR_CSS1;
BYTE color_css2=COLOR_CSS2;
BYTE color_cstec=COLOR_CSTEC;
BYTE color_cssel=COLOR_CSSEL;
BYTE color_err=COLOR_ERR;
BYTE color_errs1=COLOR_ERRS1;
BYTE color_errs2=COLOR_ERRS2;
BYTE color_errtec=COLOR_ERRTEC;
BYTE color_errsel=COLOR_ERRSEL;
...
char nf_ent[_MAX_PATH], nf_sal[_MAX_PATH];
...
c_crea_cuadro(&ccomp," COMPILADOR "COPYRIGHT" versi¢n "VERSION_CS" ",CCOMP_FIL,
  CCOMP_COL,CCOMP_ANCHO,CCOMP_ALTO,color_cs,color_css1,color_css2,color_cstec,
  color_cssel);
inpfent.cadena=nf_ent;
inpfent.longitud=_MAX_PATH-1;
inpfsal.cadena=nf_sal;
inpfsal.longitud=_MAX_PATH-1;
c_crea_elemento(&ccomp,0,0,17,CCOMP_ANCHO-19,3,color_cs,color_css2,color_css1,
  C_ELEM_INPUT,&inpfent);
c_crea_elemento(&ccomp,1,3,17,CCOMP_ANCHO-19,3,color_cs,color_css2,color_css1,
  C_ELEM_INPUT,&inpfsal);
if(cs_eds==FALSE) {
	c_crea_elemento(&ccomp,2,6,0,12,3,color_cs,color_css1,color_css2,
	  C_ELEM_BOTON,&bcomp);
	c_crea_elemento(&ccomp,3,9,0,12,3,color_cs,color_css1,color_css2,
	  C_ELEM_BOTON,&bsalir);
}

/* cuadro de di logo para lista de errores */
c_crea_cuadro(&cerr," Errores ",CERR_FIL,CERR_COL,CERR_ANCHO,CERR_ALTO,
  color_err,color_errs1,color_errs2,color_errtec,color_errsel);
c_crea_elemento(&cerr,0,0,0,CERR_ANCHO-2,CERR_ALTO-5,color_err,color_errs2,
  color_errs1,C_ELEM_LISTA,&lsterr);
c_crea_elemento(&cerr,1,CERR_ALTO-5,1,8,3,color_err,color_errs1,color_errs2,
  C_ELEM_BOTON,&bvale);
...
if(!codigo) {
	v_pon_cursor(&ccomp.v,FIL_TECLA,COL_TECLA);
	v_impcad(&ccomp.v,"Pulsa una tecla",V_RELLENA);
...
if(cs_eds==FALSE) _setvideomode(_DEFAULTMODE);
...
do {
	...
	/* si se trata de una secci¢n de m£ltiple, coge su n£mero y */
...
_splitpath(nf_ent,drive_n,dir_n,fname_n,ext_n);
...
color_cs=cfg.color_cs;
color_css1=cfg.color_css1;
color_css2=cfg.color_css2;
color_cstec=cfg.color_cstec;
color_cssel=cfg.color_cssel;
color_err=cfg.color_err;
color_errs1=cfg.color_errs1;
color_errs2=cfg.color_errs2;
color_errtec=cfg.color_errtec;
...
v_color(&ccomp.v,(BYTE)(color_cs | 0x80));
v_impcad(&ccomp.v,"<-",V_NORELLENA);
v_color(&ccomp.v,ccomp.v.attr_princ);
...
while(1) {
	err=compila_msg(tf_file[MSG],buff_compil1,memoria,(MAX_MSG-1),
	  (MAX_TMSG-1),tab_desp_msg,&nt_msg,&num_msg,&bytes_msg);

	cab.num_msg[nt_msg]=num_msg;
	cab.bytes_msg[nt_msg]=bytes_msg;

	if(err.codigo && (err.codigo!=_E_MXXX)) imp_error(err.codigo,
	  err.linea);
	...
	cab.fpos_msg[nt_msg]=ftell(f_sal);
