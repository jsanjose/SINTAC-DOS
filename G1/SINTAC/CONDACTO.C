...
STC_CONDACTO cd[]={             /* tabla funci¢n-tipo de condacto */
...
/****************************************************************************
	PAUSA: realiza una pausa. Si se pulsa una tecla, sale de la pausa,
	  ...
	  Entrada:      'wait' tiempo de la pausa en milisegundos, si es 0,
	...
	GRAPHIC: presenta un gr fico en la ventana actual.
	  Entrada:      'graf' n£mero de gr fico
			'modo' modo de dibujo
			  ...
			  1 - dibuja la imagen al final (pone toda la
			      pantalla en negro)
			  2 - como 0 pero no altera la paleta de colores
			      actual
...
if(zero(5)) {
	err=pcx_decodif16(info.name,x,y,ancho,alto,modo);
}
else {
	err=pcx_decodif256(info.name,x,y,ancho,alto,modo);
}
