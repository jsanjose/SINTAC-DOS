...
#define PCF_ICOF_ARR 4
static int cf_icof_arr[PCF_ICOF_ARR][2]={
	{7, 3}, {11, 11}, {3, 11}, {7, 3}
};

#define PCF_ICOF_ABJ 4
static int cf_icof_abj[PCF_ICOF_ABJ][2]={
	{3, 3}, {11, 3}, {7, 11}, {3, 3}
};

#define PCF_ICOF_ARRR 9
static int cf_icof_arrr[PCF_ICOF_ARRR][2]={
	{3, 11}, {7, 3}, {11, 11}, {9, 11}, {7, 5}, {5, 11},
	{3, 11}, {11, 11}, {3, 11}
};

#define PCF_ICOF_ABJR 9
static int cf_icof_abjr[PCF_ICOF_ABJR][2]={
...
while(!encontrado) {
	if(find.ff_attrib & FA_DIREC) {
		...
		strcpy(&plf2->fich[1],find.ff_name);
		/* pone corchete final */
...
encontrado=findfirst(nfich,&find,0);
...
for(i=0; i<nelem; i++) {
	moveto(x,y);
	imp_cad(lf->fich,colorf,color,CHR_NORM);
...
while(*texto) {
	...
	buf_txt[i]='\0';
	moveto(x+((ancho-(strlen(buf_txt)*8))/2),y0);
	imp_cad(buf_txt,colorf,color,CHR_NORM);
...
if(ventana->x==V_CENT) ventana->x=(getmaxx()+1-ventana->ancho)/2;
/* mira si se quiere centrar la ventana en Y */
if(ventana->y==V_CENT) ventana->y=(getmaxy()+1-ventana->alto)/2;
...
mem=imagesize(ventana->x,ventana->y,x1,y1);

/* intenta reservar la memoria para guardar fondo */
ventana->buff=malloc(mem);
...
/* guarda fondo */
getimage(ventana->x,ventana->y,x1,y1,ventana->buff);
...
putimage(ventana->x,ventana->y,ventana->buff,COPY_PUT);
...
static char rfich[MAXPATH];
char dir_orig[MAXPATH], nombre[LNG_NFICH+2];
...
b.bot[9].icono=&cf_icof_arrr[0][0];
...
b.bot[10].icono=&cf_icof_abjr[0][0];
...
moveto(b.bot[1].xb+6,b.bot[1].yb+((b.bot[1].alto-8)/2));
imp_cad(nfich,b.color_fondo,b.color_pplano,CHR_NORM);

/* construye ruta al directorio actual */
getcwd(rfich,MAXPATH);

/* la guarda */
strcpy(dir_orig,rfich);

moveto(b.bot[3].xb+6,b.bot[3].yb+((b.bot[3].alto-8)/2));
if(strlen(rfich)>(size_t)((b.bot[3].ancho-12)/8))
  impn_cad(rfich,(b.bot[3].ancho-12)/8,b.color_fondo,b.color_pplano,
    CHR_NORM);
else imp_cad(rfich,b.color_fondo,b.color_pplano,CHR_NORM);
	...
	switch(boton.boton) {
		...
		case 3 :        /* zona ruta fichero */
			...
			lee_input(b.bot[3].xb+6,
			  ...
			  rfich,MAXPATH-1,b.bot[3].ancho-12,
			  b.color_fondo,b.color_pplano);
			...
			if(*(rfich+1)==':') {
				...
				setdisk(toupper(*rfich)-'A'+1);
				if(chdir(rfich)) setdisk(i);
			}
			...
			if(i>((b.bot[3].ancho-12)/8))
			  impn_cad(rfich,(b.bot[3].ancho-12)/8,
			    b.color_fondo,b.color_pplano,CHR_NORM);
			else imp_cad(rfich,b.color_fondo,b.color_pplano,
			  CHR_NORM);
		...
		case 4 :        /* lista de ficheros */
			...
			setcolor(b.color_pplano);
			rectangle(b.bot[4].xb+6,b.bot[4].yb+(elem*10)+6,
			  b.bot[4].xb+117,b.bot[4].yb+(elem*10)+13);
				...
				*nomb_fich='\0';
				...
				if(i>((b.bot[3].ancho-12)/8))
				  impn_cad(rfich,(b.bot[3].ancho-12)/8,
				    b.color_fondo,b.color_pplano,
				    CHR_NORM);
				else imp_cad(rfich,b.color_fondo,
...
dibuja_barra_boton(&b);
imp_texto_centro(aviso,v.x,v.y+3,v.ancho,v.color_fondo,v.color_pplano);
...
/* dibuja el cuadro de aviso en pantalla */
dibuja_barra_boton(&b);
imp_texto_centro(mensaje,v.x,v.y+3,v.ancho,v.color_fondo,v.color_pplano);
