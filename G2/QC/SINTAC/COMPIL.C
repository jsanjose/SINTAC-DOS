...
while(!feof(f_msy)) {
	...
	for(j=0; j<nt; j++, ptr_act_msy++, i++) desp_msy[i]=ptr_act_msy-msy;
}

/* guarda n£mero de mensajes compilados y la memoria ocupada */
*n_msy=--i;
*bytes_msy=act_msy-msy;
...
STC_ERR compila_msg(FILE *f_msg, char *msg, unsigned mem_msg, BYTE max_msg,
  BYTE max_tmsg, unsigned *desp_msg, BYTE *nt_msg, BYTE *n_msg,
  unsigned *bytes_msg)
{
STC_ERR err={_E_NERR, 0};
...
while(!feof(f_msg)) {
	...
	/* calcula desplazamientos de mensajes, al final 'i' */
	/* contendr  el n£mero de siguiente mensaje a compilar */
	for(j=0; j<nt; j++, ptr_act_msg++, i++) desp_msg[i]=ptr_act_msg-msg;
}
...
if((indir!=_E_CIND) && (par2>lim21) && (par2<lim22)) return(coderr2);
