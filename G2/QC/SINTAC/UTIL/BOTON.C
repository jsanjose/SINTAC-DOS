...
/* recoge informaci¢n del modo de video actual */
_getvideoconfig(&vid);
...
if(!boton->icono)
  escribe_texto_boton(boton->texto_bot,boton,colorp,colort,colorf);
	...
	_setviewport(boton->xb+3,boton->yb+3,boton->xb+boton->ancho-3,
	  boton->yb+boton->alto-3);
	_polygon(_GBORDER,boton->icono,boton->nump_ico);
	/* restaura zona de dibujo a toda la pantalla */
	_setviewport(0,0,vid.numxpixels-1,vid.numypixels-1);
}
...
do {
	...
	if((cur-xcur+ancho)>fin) imp_str(cur-xcur,colorf,color,CHR_NORM);
	else impn_str(cur-xcur,ancho,colorf,color,CHR_NORM);
