...
/*** Constantes ***/
#define MVTXT	0x0003		/* modo texto 80x25 color */
#define MV16	0x0012		/* modo 640x480x16 */
#define MV256	0x0013		/* modo 320x200x256 */
...
if(num_colores==16) {
	if(!modo_video(MV16)) error_pcx(11);
}
else if(num_colores==256) {
	if(!modo_video(MV256)) error_pcx(11);
}
else error_pcx(12);
...
/****************************************************************************
	MODO_VIDEO: establece modo de v¡deo.
	  Entrada:	'mvid' modo de v¡deo a establecer
	  Salida:	1 si pudo establecer modo de v¡deo, 0 si no
****************************************************************************/
int modo_video(int mv)
{
int modovideo;
