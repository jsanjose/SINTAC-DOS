...
static void _far int24_hnd(unsigned deverror, unsigned errcode,
...
static void imprime_lista(STC_VENTANA *v, STC_LISTA *l, int elemento,
  int elemento_sel, BYTE attr_tecla, BYTE attr_sel);
...
/****************************************************************************
	IMPRIME_LISTA: imprime una lista de cadenas de un elemento de
	  ...
	  Entrada:      'v' ventana donde se imprimir  la lista
			...
			'attr_tecla' atributo de tecla de activaci¢n
...
{
void (_interrupt _far *int24_old)(void);
struct find_t find;
char disq[_MAX_DRIVE], direct[_MAX_DIR], fich[_MAX_FNAME], ext[_MAX_EXT],
  nfich[_MAX_PATH], dir[15];
int i;
unsigned encontrado;

/* instala 'handler' de errores cr¡ticos */
int24_old=_dos_getvect(0x24);
_harderr(int24_hnd);

/* separa ruta de m scara */
_splitpath(ruta_masc,disq,direct,fich,ext);
...
encontrado=_dos_findfirst(ruta_masc,_A_NORMAL,&find);

while(!encontrado) {
	/* mete nombre de fichero en lista */
	if(c_mete_en_lista(lista,find.name)==0) {
		/* restaura 'handler' de errores cr¡ticos */
		_dos_setvect(0x24,int24_old);
		return(0);
	}
...
if(fil==C_CENT) fil=(vid.numtextrows-alto)/2;
if(col==C_CENT) col=(vid.numtextcols-ancho)/2;
	...
	if(elemento>=0) {
		switch(c->elem[elemento].tipo) {
			...
			case C_ELEM_LISTA :
				...
				break;
			case C_ELEM_CHECK :
				c_elem_check(&c->elem[elemento].v,
				  c->elem[elemento].info,c,elemento);
				break;
			case C_ELEM_TEXTO :
				break;
		}
...
while(1) {
	if(e->elemento_sel!=elemento_ant) imprime_lista(v,e->elemento,
	  e->elemento_pr,e->elemento_sel,attr_tecla,attr_sel);
	...
	do {
		...
		if(r.boton1) {
			...
			if((r.fil<v->fil) || (r.fil>maxfil) ||
			  (r.col<v->col) || (r.col>maxcol)) {
				...
				imprime_lista(v,e->elemento,e->elemento_pr,
				  e->elemento_sel,attr_tecla,attr_sel);
			...
			tecla=_bios_keybrd(_KEYBRD_READY);
			/* saca tecla del buffer */
			if(tecla) _bios_keybrd(_KEYBRD_READ);
		}
...
char nfich[_MAX_PATH], disq[_MAX_DRIVE], direct[_MAX_DIR], fich[_MAX_FNAME],
  ext[_MAX_EXT], dir_orig[_MAX_PATH], dir[15];
...
nombre_fich.longitud=_MAX_PATH-1;
	...
	switch(elemento) {
		...
		case 1 :        /* lista de ficheros */
			...
			/* comprueba si es directorio o fichero */
				...
				_splitpath(nfich,disq,direct,fich,ext);
