...
extern STC_WINDOW w_deb;        /* ventana de debugger */
extern char _huge *img_debug;   /* puntero buffer para fondo ventana debug. */
#endif
...
extern char nf_base_datos[_MAX_PATH];   /* nombre de fichero base de datos */
...
extern char *tab_conx;          /* puntero inicio zona de conexiones */
...
extern unsigned char _far cardef8x16[256][16];
extern unsigned char _far cardef8x8[256][8];
...
/****************************************************************************
	PAUSA: realiza una pausa. Si se pulsa una tecla, sale de la pausa,
	  ...
	  Entrada:      'p' tiempo de la pausa en milisegundos, si es 0,
			se espera a que pulse una tecla
****************************************************************************/
...
def_chr_set(&cardef8x16[0][0],&cardef8x8[0][0]);
...
/****************************************************************************
	W_INPUTT: permite introducir por teclado una l¡nea de caracteres con
...
****************************************************************************/
unsigned w_inputt(char *plin, unsigned maxlong, char cursor, int conver,
  STC_WINDOW *w, int tiempo, int modot)
{
unsigned c, long_lin;
char *pcur, *pfinl, *i;
BYTE oldx, oldy;
...
_bios_timeofday(_TIME_GETCLOCK,&tinicio);
...
do {
	...
	if(((modot==0) || ((modot==1) && primer_car)) && tiempo) {
		...
		do {
			/* coge contador de reloj actual */
			_bios_timeofday(_TIME_GETCLOCK,&tactual);
...
if(wx==255) wx=(BYTE)((var[14]-lx)/2);
if(wy==255) wy=(BYTE)((var[15]-ly)/2);

/* ajusta para que entre en pantalla */
if(wx>(BYTE)(var[14]-1)) wx=(BYTE)(var[14]-1);
if(wy>(BYTE)(var[15]-1)) wy=(BYTE)(var[15]-1);
if((wx+lx)>var[14]) lx=var[14]-wx;
if((wy+ly)>var[15]) ly=var[15]-wy;

w_crea(wy,wx,lx,ly,cwf,cw,NO_BORDE,&w[nw]);
...
k=w_inputt(frase,MAXLONG,*(tab_msy+tab_desp_msy[7]),WINPUT_CONV,&w[var[0]],
  (int)var[16],modo_temp);

/* guarda c¢digo conque termin¢ INPUT */
var[12]=(BYTE)k;

/* si no tecle¢ nada o se pas¢ el tiempo l¡mite */
if((*frase=='\0') || (var[12]==1)) return(TRUE);
...
/****************************************************************************
	LISTOBJ: lista los objetos de la localidad actual.
	  Entrada:      bandera 7 a 1 si se imprime mensaje 'Tambi‚n puede
...
if(zero(0) || light()) {
	if(j || notzero(7)) sysmess(9);         /* 'Tambi‚n puedes ver: ' */
	listat(var[1]);
}
...
w_input(f_sl,MAXLONG,*(tab_msy+tab_desp_msy[7]),WINPUT_CONV,&w[var[0]]);
newline();
...
/* color de fondo */
w[var[0]].colortf=(col >> 4);
/* color de primer plano */
w[var[0]].colort=(col & 0x0f);
...
w_crea(w[nw].wy,w[nw].wx,w[nw].lx,w[nw].ly,w[nw].colorf,w[nw].color,borde,
  &w[nw]);
...
if(var[12]==1) return(TRUE);
else return(FALSE);

}

/****************************************************************************
	MODE: selecciona el modo de video.
...
w_crea(w_deb.wy,w_deb.wx,w_deb.lx,w_deb.ly,w_deb.colorf,w_deb.color,
  w_deb.borde,&w_deb);
...
/****************************************************************************
	GRAPHIC: presenta un gr fico en la ventana actual.
	  Entrada:      'graf' n£mero de gr fico
			'modo' modo de dibujo
	  ...
	  Salida:       TRUE si se carg¢ el gr fico con ‚xito
			FALSE si hubo alg£n error
	  NOTA: los gr ficos deben estar en ficheros cuyo nombre sea
	  G#??????.ext siendo 'ext' el n£mero identificador de 1 a 255,
	  por ejemplo: G#CASA.1, G#RIO.203, ...
****************************************************************************/
BOOLEAN graphic(BYTE graf, BYTE modo)
{
struct find_t info;
char grafico[8]="G#*.";
char num[4];
short x, y;
int ancho, alto, err, paleta=0;
...
/* busca el fichero y sale si no lo encontr¢ */
/* el nombre verdadero del fichero estar  en 'info.name' */
if(_dos_findfirst(grafico,_A_NORMAL,&info)) return(FALSE);

/* calcula posici¢n y dimensiones de la ventana actual, en pixels */
x=w[var[0]].wxi*8;
y=w[var[0]].wyi*w[var[0]].chralt;
ancho=w[var[0]].lxi*8;
alto=w[var[0]].lyi*w[var[0]].chralt;

_heapmin();

if((modo==0) || (modo==2)) {
	if(zero(5)) err=pcx_16p(info.name,x,y,ancho,alto,paleta);
	else err=pcx_256p(info.name,x,y,ancho,alto,paleta);
}
else {
	if(zero(5)) err=pcx_16(info.name,x,y,ancho,alto,paleta);
	else err=pcx_256(info.name,x,y,ancho,alto,paleta);
}
