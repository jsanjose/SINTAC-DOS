...
/*** Variables externas ***/
/* indica si se ejecuta desde el entorno */
BOOLEAN entorno;
/* ventanas del compilador y de errores */
extern STC_WINDOW wcomp;
extern STC_WINDOW werr;
/* puntero a fichero de errores */
extern FILE *ferr;
...
/****************************************************************************
	COMP_TXT: compila texto de secciones de Mensajes, Mensajes del
	  ...
	  Entrada:      'f_txt' puntero al fichero que contiene el texto
			...
			'nt' variable d¢nde se guardar  el n£mero de textos
...
{
STC_ERR err={_E_NERR, 0};
char *ult_men, *act_men, *ptr_act_men;
BYTE i=0;
int nt, j;
...
while(!feof(f_men)) {
	...
	err=comp_txt(f_men,&act_men,ult_men,i,max_men,&nt);
	...
	for(j=0; j<nt; j++, ptr_act_men++, i++) {
		/* guarda desplazamiento de mensaje */
		desp_men[i]=ptr_act_men-men;
	}
}
...
{
STC_ERR err={_E_NERR, 0};
char *ult_loc, *act_loc, *ptr_act_loc;
BYTE *act_conx, *ult_conx;
BYTE nloc=0;
char lin[LONG_LIN], *l;
long pos_fich;
char pal_conx[LONGPAL+1];
int npal, nt, i;
...
long pos_fich;
int i, npal;
char etq[LONGETQ+1], pal[LONGPAL+1], cond[LNGCOND+1];
BYTE *sgte_entr;
...
/* guarda posici¢n de cursor en ventana */
wcomp.cwxs=wcomp.cwx;
wcomp.cwys=wcomp.cwy;
...
while(1) {
	...
	wcomp.cwx=wcomp.cwxs;
	wcomp.cwy=wcomp.cwys;
	sprintf(buff,"%5u",p);
	w_imps(buff,&wcomp);
	...
	while(1) {
		...
		while(1) {
			...
			switch(condacto[i].tipo) {
				...
				case 5 :
					err.codigo=compila_par1(&l,&act_pro,
					  ult_pro,(BYTE)i,(BYTE)(n_msg-1),255,
					  _E_MPNM);
					break;
				case 6 :
					err.codigo=compila_par1(&l,&act_pro,
...
/****************************************************************************
	METE_CONST: mete una constante en la tabla de constantes.
	  Entrada:      'l' puntero a l¡nea de entrada conteniendo
			la constante
			'l¡nea' n£mero de l¡nea del fichero d¢nde se define
...
/****************************************************************************
	COMPILA_PAR1: compila condactos con un par metro.
	  Entrada:      'l' puntero al puntero a la l¡nea con el par metro
			del condacto
			'act_pro' puntero al puntero a d¢nde dejar condacto
			compilado
			'ult_pro' puntero a £ltimo byte de zona de procesos
...
par3err=compila_par2(l,act_pro,ult_pro,codigo,lim1,lim2,255,coderr1,coderr2);
...
/* compila con los dos primeros par metros */
par4err=compila_par2(l,act_pro,ult_pro,codigo,lim1,lim2,255,coderr1,coderr2);
/* si hubo error al compilarlos, sale */
if(par4err) return(par4err);

if(sgte_campo(l)) return(_E_FCAM);
/* coge el tercer par metro */
par4err=coge_num(*l,&par3);
/* si hubo error al cogerlo, sale */
if(par4err) return(par4err);
/* si est  fuera de l¡mite sale */
if(par3>lim3) return(coderr3);

if(sgte_campo(l)) return(_E_FCAM);
/* coge el cuarto par metro */
par4err=coge_num(*l,&par4);
/* si hubo error al cogerlo, sale */
if(par4err) return(par4err);
/* si est  fuera de l¡mite sale */
if(par4>lim4) return(coderr4);
...
{
COD_ERR par7err;
BYTE par3, par4, par5, par6, par7;

/* comprueba si cabe en memoria */
if(*act_pro+8>=ult_pro) return(_E_FMEM);

/* compila con los dos primeros par metros */
par7err=compila_par2(l,act_pro,ult_pro,codigo,lim1,lim2,255,coderr1,coderr2);
/* si hubo error al compilarlos, sale */
if(par7err) return(par7err);

if(sgte_campo(l)) return(_E_FCAM);
/* coge el tercer par metro */
par7err=coge_num(*l,&par3);
/* si hubo error al cogerlo, sale */
if(par7err) return(par7err);

if(sgte_campo(l)) return(_E_FCAM);
/* coge el cuarto par metro */
par7err=coge_num(*l,&par4);
/* si hubo error al cogerlo, sale */
if(par7err) return(par7err);

if(sgte_campo(l)) return(_E_FCAM);
/* coge el quinto par metro */
par7err=coge_num(*l,&par5);
/* si hubo error al cogerlo, sale */
if(par7err) return(par7err);

if(sgte_campo(l)) return(_E_FCAM);
/* coge el sexto par metro */
par7err=coge_num(*l,&par6);
/* si hubo error al cogerlo, sale */
if(par7err) return(par7err);

if(sgte_campo(l)) return(_E_FCAM);
/* coge el sexto par metro */
par7err=coge_num(*l,&par7);
/* si hubo error al cogerlo, sale */
if(par7err) return(par7err);
