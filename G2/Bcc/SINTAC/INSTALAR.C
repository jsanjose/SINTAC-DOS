...
if(filelength(forg)<cuenta) cuenta=filelength(forg);
...
while(!eof(forg)) {
	if((cuenta=read(forg,buff,(unsigned)cuenta))==-1) return(1);
	if((cuenta=write(fdest,buff,(unsigned)cuenta))==-1) return(1);
}
