...
FILE *f_in;
FILE *f_sal;
char *nombres;      /* nombre del fichero de salida */
...
char nombre[129], drive_n[_MAX_DRIVE], dir_n[_MAX_DIR],
  fname_n[_MAX_FNAME], ext_n[_MAX_EXT], *path_fsal, *nomb_fsal;
unsigned t_msg, t_msy, t_loc, t_con, t_obj, t_pro;

printf("\n"COPYRIGHT);
printf("Compilador ");
#if REDUCIDO==ON
	printf("Versi¢n "VERSION_CSR);
#else
	printf("Versi¢n "VERSION_CS);
#endif

if(argc<2) {
	printf("Nombre de fichero a compilar: ");
	gets(nombre);
...
path_fsal=strcat(drive_n,dir_n);        /* construye path */
nomb_fsal=strcat(path_fsal,fname_n);    /* construye ruta, sin ext */
nombres=strcat(nomb_fsal,".DAT");       /* ruta con extensi¢n */
if(argc<3) {
	printf("Fichero de salida [%s]: ",nombres);
	gets(ns);   /* coge nombre de fichero de salida */
	for(i=0;ns[i];i++) ns[i]=mayuscula(ns[i]);
	if(*ns!='\0') nombres=ns;   /* si introduce nombre, coge ese */
}
else {
...
do {
	...
	if(*l=='\\') {      /* ¨inicio de secci¢n? */
		...
		for(i=0;i<NS;i++) {
			if(!strcmp(seccion,n_secc[i])) {
				error=_E_NERR;  /* si encuentra marca de secci¢n */
				ant_secc=analiza;   /* guarda la secci¢n anterior */
				analiza=i;          /* y almacena secci¢n nueva */
				if(analiza!=END)
					if(analiza==PRO) {
						if(proact==0) c=coge_cur();
						pon_cur(c);
						printf("--- Compilando Proceso %u ---",
						  (unsigned)proact);
					}
					else printf("--- Compilando secci¢n de %s ---\n",
					  nomb_seccs[analiza]);
				break;
			}
		}
		if(!error) {
			if((ant_secc<PRO) && (analiza!=END)) {
				...
				/* secci¢n repetida */
				if((analiza!=END) && (analiza!=PRO) && compiled[analiza]) {
					error=_E_SCRP;
					break;
				}
			}
   ...
   switch(analiza) {
		...
		case PRO :
			error=comp_pro();
			if(error) break;
			else error=sust_labels();   /* sustituye etiquetas forward */
			break;
		case END :
			switch(ant_secc) {
				case END :
				...
				default :
					if(!compiled[ant_secc]) {   /* si no compilada */
						compiled[ant_secc]=1;   /* marca compilada */
						error=_E_NERR;          /* y desactiva error */
						switch(ant_secc) { /* ajustes finales tras compilar */
							case LOC :
								error=chequea_conx();
								break;
						}
					}
					else error=_E_MSCC; /* marca de secci¢n no encontrada */
					break;
			}
			break;
		default :
...
if(error==_E_LECF) {
	if(feof(f_in)) {    /* fin de fichero */
		if(ant_secc==PRO) error=chequea_pro();  /* acaba de compilar proc. */
...
/* si no hubo error o es fin de fich. mira si hay alguna secc. sin compilar */
	for(i=0;i<(NS-1);i++) {
		if(!compiled[i]) {
		  ...
		  sin_compilar+=1;  /* marca que hay alguna secc. sin compilar */
		}
	}
	...
	if(!sin_compilar) { /* si se compilaron todas sin error */
		if((f_sal=fopen(nombres,"wb"))==NULL) {
			printf("\nError de apertura de fichero de salida [%s].\n",
			  nombres);
			exit(1);
		}
...
/*****************************************************************************
	LEE_LINEA: lee lineas del fichero de entrada hasta encontrar una que
	  ...
	  Usa la variable global *l que es un puntero al buffer de entrada lin[]
*****************************************************************************/
int lee_linea(void)
{
l=lin;
...
/*****************************************************************************
	COGE_PAL: recoge una palabra de vocabulario de la linea de entrada l y la
	  mete en *pal (buffer de al menos LONGPAL caracteres).
	  Devuelve:
		_E_FCAM si encontr¢ un car. fin de linea (no hay m s campos detr s
		  de la palabra).
...
for(i=0;i<LONGPAL-1;i++) {
	...
	if((*l==' ') || (*l=='\t')) {   /* si encuentra espacio a tab. */
		...
		*(pal+i)=mayuscula(*l);
		...
		if(!esta_en(ABECEDARIO,*(pal+i)) && !esta_en(NUMEROS,*(pal+i)))
		  return(_E_CVOC);
	}
}
*(pal+i)='\0';
...
/*****************************************************************************
	COMP_TXT: compila texto de secciones de Mensajes, Mensajes del Sistema,
	  Localidades...
	  Devuelve un valor distinto de 0 si hubo error.
*****************************************************************************/
int comp_txt(char *l0,ptrdiff_t *mens,char far *(*tmsg), char far *(*actm),
...
do {
	...
	if(elin=lee_linea()) return(elin); /* error de lectura o fin de fichero */
	l0=lin;
...
do {
	...
	if(fgetpos(f_in,pos)) return(_E_FENT); /* almacena posici¢n en fichero */
	if(errct=lee_linea()) return(errct);
	if(*l!='#') {   /* si no es marca de conexiones */
		if(fsetpos(f_in,pos)) return(_E_FENT); /* restaura pos. en fichero */
		numlin=numlin0; /* restaura n£mero de l¡nea */
		*act_conx++=(BYTE)0;  /* indica fin de conexiones para localidad */
		...
		l=hasta_espacio(l);
		l=salta_espacios(l);
		if(errct=coge_pal(con)) return(errct); /* coge el campo palabra */
		if((palfound=esta_en_voc(con))==(NUM_PAL+1)) return(_E_NPVC);
		num=vocabulario[palfound].num;
		tipo=vocabulario[palfound].tipo;
		if(((tipo!=_VERB) && (tipo!=_NOMB)) || (num>=v_mov)) return(_E_NMOV);
		l=hasta_espacio(l); /* desprecia cars. hasta 1er espacio o tab. */
		l=salta_espacios(l);
		if(coge_num(&nloc,l)) return(_E_CNUM); /* error campo num‚rico */
		if(nloc>(MAX_LOC-1)) return(_E_NVAL); /* n£m. loc. no v lido */
...
if((loc_ini<NO_CREADO) && (loc_ini>=n_loc)) return(_E_LNVL);
...
do {
	if(*l=='\\') {  /* si encuentra marca de secci¢n */
		...
		s[i]='\0';
		if(strcmp(s,n_secc[_END])) return(_E_MEND); /* END no encontrado */
		else {                  /* si encontr¢ fin de proceso */
			proact++;           /* siguiente n£mero de proceso */
			*proc_act++=0;      /* pone marcas de fin de proceso */
			if(proc_act==proc_ult) return(_E_FMEM);     /* fuera de mem. */
			*proc_act++=0;
			if(proc_act==proc_ult) return(_E_FMEM);     /* fuera de mem. */
			compiled[_PRO]=1;   /* marca de compilado al menos 1 proc. */
			return(_E_NERR);    /* retorna sin error-->proc. xx compilado */
		}
	}
	...
	if(*pal=='_') {     /* si es no-palabra */
		*proc_act++=NO_PAL;
		...
		if((vocabulario[i].tipo!=_VERB) &&
		  ...
		  return(_E_NVNC);
		*proc_act++=vocabulario[i].num; /* guarda num. de verbo-nomb.conv. */
	}
	...
	if(proc_act==proc_ult) return(_E_FMEM);     /* comprueba fuera de mem. */
	sgte_entr=proc_act; /* guarda ptro. a d¢nde meter desplz. sgte. entr. */
	proc_act+=2;        /* salta 2 bytes para empezar a compilar entrada */
	if(proc_act>=proc_ult) return(_E_FMEM);     /* comprueba fuera de mem. */
	/* espera encontrar un condacto */
	pra_lin_entr=1; /* estamos en primera linea de entrada */
	do {
		if(pra_lin_entr) {
			l=hasta_espacio(l);   /* necesario por si es 1era lin. entrada */
			l=salta_espacios(l);  /* deja ptro. al inicio de condacto */
			pra_lin_entr=0;
		}
			...
			/* si 1er car. no es espacio o encuentra marca de secci¢n */
			if(((lin[0]!=' ') && (lin[0]!='\t')) || (*l=='\\')) {
				*proc_act++=0;  /* marca fin de entrada */
				if(proc_act==proc_ult) return(_E_FMEM); /* fuera de mem. */
				/* guarda desplz. sgte. entrada respecto a inicio proc. */
				se=(proc_act-proc)-pro[proact];
				*sgte_entr=(BYTE)(se & 0x00ff);             /* byte bajo */
				*(sgte_entr+1)=(BYTE)((se >> 8) & 0x00ff);  /* byte alto */
		...
		for(i=0;i<N_CONDACTOS+1;i++) {  /* busca condacto */
			if(!strcmp(cond,condactos[i])) break;   /* si lo encuentra sale */
		}
		/* retorna con error si condacto no encontrado o no v lido */
		if(i==N_CONDACTOS+1) return(_E_NCND);
		/* si no lo compila comprobando par metros, indirecci¢n, etc... */
...
/*****************************************************************************
	CHECK_MEM: comprueba si hay memoria suficiente para compilar un condacto.
...
/*****************************************************************************
	SUST_LABELS: sutituye referencias forward en saltos SKIP.
	  Devuelve: _E_NERR si no hubo error
				_E_RFFW si etiqueta no definida y 'numlin' n£mero de l¡nea
				del error
...
for(i=0;i<ptskip;i++) {
	for(j=0;j<ptlab;j++) if(!strcmp(fskip[i].label,etiqueta[j].label)) break;
	if(j==ptlab) {  /* si no encontr¢ la etiqueta */
		numlin=fskip[i].nl; /* num. de lin. d¢nde est  error */
		return(_E_RFFW);
	}
	/* si la etiqueta est  definida */
	desplu=(unsigned)(etiqueta[j].plab-(fskip[i].fsk-1)); /* desplazamiento */
...
if(wx>COLUMNAS-1) return(_E_WPWX);
if(sgte_campo()) return(_E_FCAM);
if(coge_num(&wy,l)) return(_E_CNUM);
if(wy>FILAS-1) return(_E_WPWY);
if(sgte_campo()) return(_E_FCAM);
if(coge_num(&lx,l)) return(_E_CNUM);
if(wx+lx>COLUMNAS-1) return(_E_WPFP);
if(sgte_campo()) return(_E_FCAM);
if(coge_num(&ly,l)) return(_E_CNUM);
if(wy+ly>FILAS-1) return(_E_WPFP);
...
if(!ptlab) {  /* si no hay ninguna etiqueta definida */
	...
	if(i==ptlab) {  /* si no encontr¢ la etiqueta */
		if(mete_fsk(lb,proc_act+1)) return(_E_RBTS); /* guarda ref. forward */
		*proc_act++=32;
		*proc_act++=0;  /* inicializa a 0, pero luego ser  sustituido */
		*proc_act++=0;  /* por su desplazamiento correspondiente */
	}
		...
		*proc_act++=32;
		desplu=(unsigned)(proc_act-1-etiqueta[i].plab); /* desplaz. atr s */
...
if(*pal=='_') {         /* si es no-palabra */
	...
	if(errct==_E_CVOC) return(errct);   /* sale si hubo palabra no v lida */
	if((i=esta_en_voc(pal))==NUM_PAL+1) return(_E_NPVC); /* no en voc. */
	if(vocabulario[i].tipo!=_NOMB) return(_E_NNNN); /* no es nombre */
