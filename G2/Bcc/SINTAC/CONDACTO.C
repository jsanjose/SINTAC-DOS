...
extern unsigned char far cardef8x16[256][16];
extern unsigned char far cardef8x8[256][8];
...
if(modovideo==0) {
	asm {
		mov ax,0600h	// borrar ventana de pantalla
		mov bh,0
		mov cx,0
		mov dh,MODO0_FIL-1
		mov dl,MODO0_COL-1
...
var[nv]=hora.ti_sec;
/* minutos de 0 a 59 */
var[nv+1]=hora.ti_min;
/* hora de 0 a 23 */
var[nv+2]=hora.ti_hour;
...
tam_img_debug=blq_tam(0,0,(w_deb.lx*8)-1,(w_deb.ly*w_deb.chralt)-1);
...
if((modo==0) || (modo==2)) {
	if(zero(5)) err=pcx_16p(info.ff_name,x,y,ancho,alto,paleta);
	else err=pcx_256p(info.ff_name,x,y,ancho,alto,paleta);
}
else {
	if(zero(5)) err=pcx_16(info.ff_name,x,y,ancho,alto,paleta);
	else err=pcx_256(info.ff_name,x,y,ancho,alto,paleta);
}
