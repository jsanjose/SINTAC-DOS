...
#if DEBUGGER==0
	#pragma message("Compilando int‚rprete...")
#else
	#pragma message("Compilando int‚rprete-debugger...")
#endif
...
cls();
if(entorno==FALSE) {
	w_crea(WJSJ_FIL,WJSJ_COL,WJSJ_ANCHO,WJSJ_ALTO,3,0,BORDE_3,&w_jsj);
	/* centra ventana */
...
while(1) {
	...
	while(res_pro && *ptr_proc) {
		...
		if(debugg==TRUE) debugger(indir1,indir2,pro_d);
		...
		switch(cd[*ptr_proc].tipo) {
			...
			case 3 :        /* BYTE,BYTE,BYTE */
				...
				ptr_proc+=4;
				break;
			case 4 :        /* BYTE,BYTE,BYTE,BYTE */
				res_pro=cd[*ptr_proc].cond(par1,par2,
				  *(ptr_proc+3),*(ptr_proc+4));
				ptr_proc+=5;
				break;
			case 6 :        /* 7 BYTE */
				res_pro=cd[*ptr_proc].cond(par1,par2,
				  *(ptr_proc+3),*(ptr_proc+4),*(ptr_proc+5),
				  *(ptr_proc+6),*(ptr_proc+7));
				ptr_proc+=8;
				break;
		}
	}
...
switch(condacto[*ptr_proc].tipo) {
	...
	case 26 :
		...
		else sprintf(lin_cond,"%3u",*(ptr_proc+1));
		break;
	case 4 :
	case 9 :
	case 15 :
	...
	case 28 :
		...
		else if(indir1) sprintf(lin_cond,"[%3u] %3u",*(ptr_proc+1),
		  *(ptr_proc+2));
		else if(indir2) sprintf(lin_cond,"%3u [%3u]",*(ptr_proc+1),
		  *(ptr_proc+2));
		else sprintf(lin_cond,"%3u %3u",*(ptr_proc+1),*(ptr_proc+2));
		break;
	case 11 :
	...
	case 2 :
		...
		else if(indir1) sprintf(lin_cond,"[%3u] %3u ",
		  *(ptr_proc+1),*(ptr_proc+2));
		else if(indir2) sprintf(lin_cond,"%3u [%3u] ",*(ptr_proc+1),
		  *(ptr_proc+2));
		else sprintf(lin_cond,"%3u %3u ",*(ptr_proc+1),*(ptr_proc+2));
		w_imps(lin_cond,&w_deb);
		sprintf(lin_cond,"%3u %3u %3u %3u %3u",*(ptr_proc+3),
		  *(ptr_proc+4),*(ptr_proc+5),*(ptr_proc+6),*(ptr_proc+7));
		break;
	case 16 :
	...
	case 21 :
		if(indir1 && indir2) sprintf(lin_cond,"[%3u] [%3u]",
		  ...
		  *(ptr_proc+2));
		  ...
		  w_imps(lin_cond,&w_deb);
		sprintf(lin_cond," %3u",*(ptr_proc+3));
		break;
	case 29 :
		if(indir1 && indir2) sprintf(lin_cond,"[%3u] [%3u]",
		  *(ptr_proc+1),*(ptr_proc+2));
		  ...
		  *(ptr_proc+2));
		  ...
		  w_imps(lin_cond,&w_deb);
		sprintf(lin_cond," %3u %3u",*(ptr_proc+3),*(ptr_proc+3));
		break;
}
...
/* -- Mensajes -- */
if((tab_msg=(char *)malloc((size_t)cab.bytes_msg))==NULL) m_err(6,errmem,1);
...
if(fread(buff,tam,cant,f_in)!=cant) {
	if(feof(f_in)) return;
	if(ferror(f_in)) m_err(5,"Error en fichero de entrada",1);
}
