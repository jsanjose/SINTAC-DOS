...
char comando[129], ant_dir[_MAX_DIR];
...
else if((buf=(char *)malloc(tam))==NULL) {
...
if(flng) while(!feof(fsecc)) {
	tam=fread(buf,sizeof(char),tam,fsecc);
	if(ferror(fsecc)) {
		imprime_error(Merr_Lect);
		fclose(fsecc);
		if(!bufaux_usado) free(buf);
	...
	if(fwrite(buf,sizeof(char),tam,fbd)!=tam) {
		imprime_error(Merr_Escr);
		fclose(fsecc);
		if(!bufaux_usado) free(buf);
		return(0);
...
int compila_bd(void)
{
char fichero_csal[_MAX_PATH], *argum[5], fichero_ed[_MAX_PATH];
int error=0;
...
int linka_bd(void)
{
char fichero_dat[_MAX_PATH], fichero_exe[_MAX_PATH], *argum[5],
  fichero_ed[_MAX_PATH];
int error=0;
...
char fichero_dat[_MAX_PATH], drive_n[_MAX_DRIVE], dir_n[_MAX_DIR],
  fname_n[_MAX_FNAME], ext_n[_MAX_EXT], ant_dir[_MAX_DIR], *argum[5],
  fichero_ed[_MAX_PATH];
...
if(access(fichero_dat,0)){
	...
	/* si fecha de fichero de base de datos es posterior a fecha */
	/* de fichero de base de datos compilada, o base de datos a sido */
	/* modificada, recompila */
...
_splitpath(base_datos,drive_n,dir_n,fname_n,ext_n);
ant_drive=_getdrive();
getcwd(ant_dir,_MAX_DIR-1);
_chdrive((int)(toupper(*drive_n)-'A'+1));
/* anula '\' final */
i=strlen(dir_n)-1;
...
STC_MENU *mutil;
FILE *futil;
char nf_utl[_MAX_PATH], lin[81], menu_util[1024], *l, ruta[MAX_NUTILS][129],
