...
if(argc<2) m_err(7,1,"Falta nombre de fichero.",1);
	...
	if(argc==3) {       /* si tecle¢ algo detr†s del nombre del fichero */
		...
		/* si introdujo /l o /L */
		/* recoge los dos siguiente d°gitos y calcula l°nea de la */
...
cls();
w_cls(&w_jsj);
w_imps("   "COPYRIGHT,&w_jsj);
w_imps("   IntÇrprete-Debugger Versi¢n "VERSION_IS,&w_jsj);
w_imps("            Pulsa una tecla.",&w_jsj);
lee_tecla();
cls();
pon_cur(0,0);
#endif
...
do {
	if(*ptr_proc) {     /* si no es fin de proceso */
		res_pro=!resp_act || (resp_act && ((*ptr_proc==NO_PAL) ||
		  ...
		  (*(ptr_proc+1)==var[3])));   /* FALSE si no debe ejec. entrada */
		ptr_proc+=2;    /* salta verbo-nombre */
		sgte_ent=(ptrdiff_t)*ptr_proc;  /* desplaz. sgte. entrada */
		ptr_proc+=2;
#if DEBUGGER==ON
	...
	while(res_pro && *ptr_proc) {
			par1=*(ptr_proc+1); /* supone que no hay indirecci¢n */
			par2=*(ptr_proc+2);
#if DEBUGGER==ON
			indir1=0;
			indir2=0;
			pro_d=ptr_proc; /* guarda direcci¢n de condacto en curso */
#endif
			if(*ptr_proc==INDIR1) { /* indirecci¢n en 1er par†metro */
			...
			else if(*ptr_proc==INDIR2) {    /* indirecci¢n en 2ß par†metro */
				ptr_proc++; /* salta prefijo */
				par1=*(ptr_proc+1);
				par2=var[*(ptr_proc+2)];
#if DEBUGGER==ON
			...
			else if(*ptr_proc==INDIR12) {   /* indirecci¢n en ambos */
				ptr_proc++; /* salta prefijo */
				par1=var[*(ptr_proc+1)];
				par2=var[*(ptr_proc+2)];
#if DEBUGGER==ON
...
if(debugg) {    /* si est† activado paso a paso */
			w_cls(&w_deb);
			sprintf(lin_cond,
			  "Proceso: %3u   ## V=variables  B=banderas"
			  "  S=salir  Otra=ejecutar ##\n",(unsigned)pro_act);
			w_imps(lin_cond,&w_deb);
			imp_condacto();     /* imprime condacto en curso */
			key=lee_tecla();    /* espera que se pulse una tecla */
			if(key==COD_F1) debugg=0;   /* desactiva el paso a paso */
			while(esta_en("vVbBsS",key)) {
				switch(key) {
					...
					case 'V' :
						do {
							sprintf(lin_cond,
							  "Variable: %3u=%3u  ..... ## O=otra"
							  "  M=modif.  S=salir ##",variable,var[variable]);
							...
							if(palabra==(NUM_PAL+1))
							  ...
							  sprintf(lin_cond,"%s",vocabulario[palabra].p);
							w_deb.cwx=34;   /* posiciona cursor */
							w_imps(lin_cond,&w_deb);
					...
					case 'B' :
						do {
							sprintf(lin_cond,
							  "Bandera: %3u=%1u  ## O=otra  M=modificar"
							  "  S=salir ##   ",bandera,notzero(bandera));
							w_deb.cwy=0;    /* posiciona cursor */
							w_deb.cwx=15;
							w_imps(lin_cond,&w_deb);
							key=lee_tecla();    /* espera tecla */
			...
			w_deb.cwy=0;    /* posiciona cursor */
			w_deb.cwx=15;
			w_imps("## V=variables  B=banderas  S=salir  Otra=ejecutar ##   ",
			  &w_deb);
			key=lee_tecla();
			}
}
/*****************************************************/
/* Eliminado porque interfiere con el condacto INKEY */
/******************************************************
else {
	if(_bios_keybrd(_KEYBRD_READY))
	  if(_bios_keybrd(_KEYBRD_READ)==F1) debugg=1;
}
******************************************************/
#endif
			switch(cd[*ptr_proc].tipo) {    /* ejecuta condacto seg£n tipo */
...
switch(c[*ptr_proc].tipo) {
	...
	case 7 :
		if(*(ptr_proc+1)==NO_PAL) sprintf(lin_cond,"-      ");
			...
			j=saca_pal(*(ptr_proc+1),_VERB);    /* quiz† sea nomb conv */
		...
		if(*(ptr_proc+2)==NO_PAL) sprintf(lin_cond,"-");
		  ...
		  sprintf(lin_cond,"%s",vocabulario[saca_pal(*(ptr_proc+2),_NOMB)].p);
		break;
	case 8 :
		if(indir1 && indir2)
		  sprintf(lin_cond,"[%3u] [%3u]",*(ptr_proc+1),*(ptr_proc+2));
...
if(w->cwx>(w->lx-1)) {  /* si llega a la £ltima columna */
	...
	if(w->cwy>(w->ly-1)) {
		...
		if(w->scroll==1) {  /* espera tecla antes de scroll */
			pon_cur(w->wy+w->ly-1,w->wx+w->lx-1);   /* esq. inf. der. */
			reg.h.ah=0x08;  /* coger caracter y attr. de pos. cursor */
			reg.h.bh=0;     /* p†gina 0 */
			int86(0x10,&reg,&reg);  /* AH=atributo, AL=car†cter */
			do {
				impc(CAR_SCROLL,w->colort); /* imp. car. de espera scroll */
				pausa(SCROLL_PAU);
				impc(reg.h.al,w->colort);   /* borra car. de espera */
				pausa(SCROLL_PAU);
...
do {
	...
	/* si se encontr¢ fin de frase, espacio o av. linea */
		...
		/* si cuenta==0, no hay palabra almacenada */
			...
			/* si la palabra almacenada no cabe en lo que queda de linea */
			/* pero cabe en la siguiente, la imprime en la sgte linea */
			/* si no, la imprime sin m†s */
			if((cuenta>w->lx-w->cwx-1) && (cuenta<=w->lx)) w_impc('\n',w);
		...
		if(*s!='_') {
			b[cuenta]=*s++; /* si letra no es espacio ni av. lin. la guarda */
			cuenta++;
			...
			while(*pto) {
				...
				if(cuenta==MAX_PAL) { /* si se llena buffer */
					...
					for(i=0;i<MAX_PAL;i++) w_impc(b[i],w);
					cuenta=0;
					/* imprime resto de palabra de objeto*/
					while((*pto!='\0') && (*pto!=' ') && (*pto!='\n')) {
						w_impc(*pto,w);
						pto++;
					}
...
while((c=lee_tecla())!=RETURN) {
	switch(c) {
		case BACKSPACE :            /* borrado */
			if(pcur!=plin) {        /* comprueba que no est† a la izquierda */
				pcur--;
		...
		case COD_IZQ :              /* mover cursor izquierda */
			if(pcur!=plin) pcur--;  /* comprueba que no estÇ a la izquierda */
			break;
		case COD_DER :              /* mover cursor derecha */
			if(pcur!=pfinl) pcur++; /* comprueba que no estÇ a la derecha */
			break;
		case COD_ARR :              /* ignora cursor arriba-abajo */
		case COD_ABJ :
			break;
		case COD_F1 :               /* si F1, activa-desactiva paso a paso */
#if DEBUGGER==ON
			if(debugg) debugg=0;
			else debugg=1;
#endif
...
unsigned tecla;
do {
	tecla=_bios_keybrd(_KEYBRD_READ);   /* byte bajo=ASCII, byte alto=CODIGO*/
...
for(i=0;i<(int)objact;i++) {    /* inicializa tabla de localidades actuales */
	...
	loc_obj[i]=(BYTE)*(po+2);   /* coge loc. inicial y la guarda en tabla */
}
	...
	ANALIZA: analiza una palabra.
	  ...
	  Salida:  actualiza tipo y num de acuerdo al vocabulario.
		Devuelve SEPARADOR si encontr¢ un separador
				 ...
				 NO_PAL si encontr¢ palabra pero no est† en vocabulario
*****************************************************************************/
int analiza(char *(*pfrase),BYTE *tipo,BYTE *num)
{
int i=0;
...
do {
	...
	if(res==PALABRA) {  /* si es palabra sin terminaci¢n */
		...
		if((tipo==_NOMB) && (f[_NOMB]<2)) { /* s¢lo coge los dos 1ß nombres */
			if(!f[_NOMB]) {  /* almacena nombre 1 en var. 3 */
				var[3]=num;
				/* si es nomb. conv. tambiÇn lo almacena en verbo */
				/* si no cogi¢ antes ning£n verbo ni otro nomb. conv. */
				if((num<n_conv) && !f[_VERB]) {
					var[2]=num;
					f[_VERB]=2; /* indica que hay nombre convertible */
					f[_NOMB]--; /* luego lo incrementar† */
				}
			}
			if(f[_NOMB]==1) var[5]=num; /* almacena nombre 2 en var. 5 */
			f[_NOMB]++; /* increm. num. nombres cogidos */
		}
		if((tipo==_ADJT) && (f[_ADJT]<2)) { /* s¢lo coge los dos 1ß adjet. */
			if(!f[_ADJT]) var[4]=num;   /* almacena adjetivo 1 en var. 4 */
			if(f[_ADJT]==1) var[6]=num; /* almacena adjetivo 2 en var. 6 */
	...
	if(res==TERMINACION) {  /* si es palabra con terminaci¢n */
		if((tipo==_VERB) && f[_VERB]!=1) {
			...
			if(nombrelo>=n_prop) {  /* si nombre anterior no era Propio */
				var[3]=nombrelo;
				var[4]=adjtlo;
...
if(res==FIN_FRASE) {
	if(!mas_texto) {
		...
		mas_texto=0;    /* si analiz¢ lo £ltimo cogido antes de fin frase */
...
/*****************************************************************************
	DEFWIN: define una ventana.
	  Entrada: nw    num. de ventana
			   cw    color
			   wx,wy posici¢n de esquina superior izquierda (columna, fila)
...
if(((nloc==COGIDO) || (nloc==PUESTO)) && (locobj!=COGIDO) && (locobj!=PUESTO))
  objs_cogidos++;
/* si el objeto estaba cogido o puesto y no lo pasa a cogido ni a puesto */
/* decrementa el num. de objetos que lleva */
if(((locobj==COGIDO) || (locobj==PUESTO)) && (nloc!=COGIDO) && (nloc!=PUESTO))
  objs_cogidos--;
...
if(!j) sysmess(10);     /* si no se list¢ ning£n objeto */
	...
	for(i=0;i<j;i++) {          /* recupera objetos guardados */
		...
		if(notzero(1)) {        /* si flag 1=1, columna */
			...
			else if(i==(j-2)) sysmess(12);  /* uni¢n entre los 2 £ltimos */
...
for(i=0;i<objact;i++) {
	...
	if((var[3]==(BYTE)*po) && ((var[4]==NO_PAL) || (var[4]==(BYTE)*(po+1)))) {
		var[8]=i;
		break;
	}
}
...
/*****************************************************************************
	MOVE: actualiza el contenido de una variable de acuerdo a su contenido
	  ...
	  contenga el num. de localidad con la que conecta una dada por medio de
	  la palabra de movimiento de la sentencia l¢gica.
	  Entrada: nv num. de variable (cuyo contenido es el num. de una loca-
				  lidad v†lida)
			   var[2] y var[3] sentencia l¢gica
	  Salida:  TRUE si hay conexi¢n y nv modificada para que contenga un num.
			   de localidad que conecta con la dada por medio de la tabla de
			   conexiones
			   FALSE si no hay conexi¢n en esa direcci¢n y nv sin modificar
*****************************************************************************/
int move(BYTE nv)
{
char far *pc;
...
while(*pc) {
	if((var[2]==*pc && var[3]==NO_PAL) ||
	  (var[2]==NO_PAL && var[3]==*pc) || (var[2]==*pc && var[3]==*pc)) {
		var[nv]=*(pc+1);    /* guarda num. de loc. hacia la que conecta */
		return(TRUE);       /* y sale */
	}
...
/*****************************************************************************
	ISMOV: comprueba si la sentencia l¢gica es de movimiento (movim. - NO_PAL,
	  NO_PAL - movim. o movim. - movim.).
...
/*****************************************************************************
	INKEY: coloca en las variables 9 y 10 el par de c¢digos ASCII IBM
	  ...
	  Salida: TRUE si se puls¢ una tecla y adem†s...
			  variable 9 conteniendo 1er c¢digo ASCII IBM (c¢digo ASCII
			  del caracter, si es distinto de 0)
				 "     10     "      2o    "      "    "  (c¢digo de scan
			  de la tecla pulsada)
...
if(_bios_keybrd(_KEYBRD_READY)) {   /* si hay tecla esperando */
	tecla=_bios_keybrd(_KEYBRD_READ);   /* recoge c¢digos de tecla pulsada */
...
for(i=VARS+BANDS;i<VARS+BANDS+MAX_OBJ;i++) loc_obj[i]=pram[i];
return(TRUE);
}
...
/*****************************************************************************
	ISDOALL: comprueba si se est† ejecutando un bucle DOALL (FIRSTO ha sido
	  ejecutado y NEXTO no ha alcanzado todav°a £ltimo objeto).
...
/*****************************************************************************
	ASK: realiza una pregunta y espera hasta que se introduce la respuesta.
	  Entrada: smess1 mensaje del sistema que contiene la pregunta
			   smess2    "     "     "    con las posibles respuestas (cada
					  una de una sola letra y seguidas, no importa si en
					  may£sculas o min£sculas), el m†ximo n£mero de carac-
					  teres permitidos es 256 (0 a 255), si son m†s la varia-
					  ble varno contendr† resultados imprevisibles
	  Salida:  varno  n£m. de variable que contendr† la respuesta (0 si se
					  tecle¢ 1er car†cter de smess2, 1 si el 2o, 2 si el
					  3ero, etc...)
...
do {
	...
	while(*pm) {
		if(mayuscula(*pm)==(char)key) {
			var[varno]=i;   /* almacena n£mero de caracter de smess2 */
			return(TRUE);   /* y sale */
		}
...
/*****************************************************************************
	END: pregunta para otra partida o salir. Si se responde con el primer
	  caracter del mensaje del sistema 25, ejecuta un EXIT 0 (reiniciali-
...
/*****************************************************************************
	PRINTAT: coloca el cursor en una posici¢n dada de la ventana actual.
	  Entrada: y,x coordenada del cursor (fila, columna); si salen de los
			   l°mites de la ventana el cursor se colocar† en la esquina
...
/*****************************************************************************
	INK: selecciona el color temporal de la tinta en la ventana activa.
	  Todos los textos de esa ventana se imprimir†n con ese color de tinta.
...
/*****************************************************************************
	PAPER: selecciona el color temporal del fondo en la ventana activa.
	  Todos los textos de esa ventana se imprimir†n con ese color de fondo.
	  Entrada: color n£mero del color 0 a 7
*****************************************************************************/
int paper(BYTE color)
{
...
/*****************************************************************************
	BRIGHT: selecciona el atributo de brillo temporal para la ventana activa.
	  Todos los textos de esa ventana se imprimir†n con ese brillo.
...
/*****************************************************************************
	BLINK: selecciona el atributo de parpadeo temporal para la ventana activa.
	  Todos los textos de esa ventana se imprimir†n con ese parpadeo.
