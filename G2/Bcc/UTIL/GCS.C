...
{
STC_PUNTERO puntero;
int i, driverg=EGA, modog=EGAHI;

if(registerbgidriver(EGAVGA_driver)<0) {
	printf("\nError de inicializaci¢n de sistema gr fico.\n");
	exit(1);
}

/* selecciona modo gr fico */
initgraph(&driverg,&modog,"");
if(graphresult()!=grOk) {
	printf("\nEste programa requiere tarjeta gr fica EGA o VGA.\n");
	exit(1);
}
...
bar(barra[0].bot[5].xb+3,barra[0].bot[5].yb+3,
...
setfillstyle(SOLID_FILL,COLOR_F_CUAD);
bar(x,y,x+(CUAD_ANCHO*8)-1,y+(CUAD_ALTO*altura_cuad)-1);

y0=y;

/* color de borde de cuadr¡cula */
setcolor(COLOR_B_CUAD);
for(i=0; i<altura_cuad; i++, y0+=CUAD_ALTO) {
	x0=x;
	for(j=0; j<8; j++, x0+=CUAD_ANCHO) rectangle(x0,y0,x0+(CUAD_ANCHO-1),
	  y0+(CUAD_ALTO-1));
}
...
if(getpixel(cuadx+1,cuady+1)==COLOR_F_CUAD) {
	setfillstyle(SOLID_FILL,COLOR_R_CUAD);
	bar(cuadx+1,cuady+1,cuadx+(CUAD_ANCHO-2),cuady+(CUAD_ALTO-2));
}
else {
	setfillstyle(SOLID_FILL,COLOR_F_CUAD);
	bar(cuadx+1,cuady+1,cuadx+(CUAD_ANCHO-2),cuady+(CUAD_ALTO-2));
...
/****************************************************************************
	IMP_CHRDEF: imprime la definici¢n de un car cter en la posici¢n
	  ...
	  la funci¢n moveto(), o por alguna otra funci¢n gr fica.
...
setwritemode(1);
setcolor(COLOR_R_ASCII);
rectangle(orgx+x-1,orgy+y-1,orgx+x+8,orgy+y+altura_cuad);
...
for(i=0; i<altura_cuad; i++, y+=CUAD_ALTO) {
	...
	for(j=0; j<8; j++, mascara>>=1, x+=CUAD_ANCHO) {
		...
		if(mascara & c) setfillstyle(SOLID_FILL,COLOR_R_CUAD);
		else setfillstyle(SOLID_FILL,COLOR_F_CUAD);
		bar(x+1,y+1,x+(CUAD_ANCHO-2),y+(CUAD_ALTO-2));
	}
}
...
moveto(barra[0].bot[5].xb+7,barra[0].bot[5].yb+7+((16-altura_cuad)/2));
...
setfillstyle(SOLID_FILL,barra[0].color_fondo);
bar(barra[0].bot[3].xb,barra[0].bot[3].yb,
  barra[0].bot[3].xb+barra[0].bot[3].ancho-1,
  barra[0].bot[3].yb+barra[0].bot[3].alto-1);
bar(barra[0].bot[4].xb,barra[0].bot[4].yb,
...
moveto(v.x+((v.ancho-160)/2),v.y+4);
imp_cad("Prueba de caracteres",v.color_fondo,v.color_pplano,CHR_NORM);
moveto(v.x+((v.ancho-240)/2),v.y+altura_cuad+18);
imp_cad("Teclea los caracteres y cuando",v.color_fondo,v.color_pplano,
  CHR_NORM);
moveto(v.x+((v.ancho-176)/2),v.y+altura_cuad+26);
imp_cad("termines pulsa RETURN.",v.color_fondo,v.color_pplano,CHR_NORM);

/* zona de prueba */
setfillstyle(SOLID_FILL,COLOR_F_PRB);
bar(v.x+9,v.y+13,v.x+(NPRB_CAR*8)+11,v.y+altura_cuad+14);

xcar=v.x+11;

do {
	/* imprime cursor */
	setcolor(COLOR_P_PRB);
	moveto(xcar,v.y+altura_cuad+13);
	lineto(xcar+7,v.y+altura_cuad+13);
