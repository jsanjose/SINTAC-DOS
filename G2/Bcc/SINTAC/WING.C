...
/*** Variables globales ***/
/* lista de teclas admitidas por la funci¢n w_lee_tecla() */
...
static int w_esta_en(char *s, char c);
...
asm {
	mov ax,0f00h    // buscar modo de v¡deo
...
if(modovideo==0x12) w->chralt=16;
/* VGA en modo 320*200 a 256 colores, altura caracteres 8 */
if(modovideo==0x13) w->chralt=8;
...
if(c!='\n') {
	imp_chr_pos((w->wxi+w->cwx)*8,(w->wyi+w->cwy)*w->chralt);
...
if(w->cwx>(BYTE)(w->lxi-1)) {
	...
	if(w->cwy>(BYTE)(w->lyi-1)) {
		...
		if(w->scroll==1) {
			...
			blq_coge(esqx,esqy,esqx+7,esqy+w->chralt-1,
