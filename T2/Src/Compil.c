	...
	while(1) {
		...
		while(1) {
			...
			switch(condacto[i].tipo) {
				...
				case 24 :
					err.codigo=compila_par2(&l,&act_pro,
					  ult_pro,(BYTE)i,FILAS-1,COLUMNAS-1,
					  255,_E_EFIL,_E_ECOL);
					break;
				case 25 :
					err.codigo=compila_par1(&l,&act_pro,
					  ult_pro,(BYTE)i,7,255,_E_COLR);
					break;
				case 26 :
					err.codigo=compila_par1(&l,&act_pro,
			...
			if(!fin_linea(*l) && *l!=';') {
				printf("\n");
				aviso("demasiados campos, se ignoran los"
				  " sobrantes, ");
				printf(Lin_Msg,err.linea);
				printf(Men_Pro);
			}
...
for(i=0; i<pt_const; i++) {
	if(!strcmp(cnst,constante[i].cnst)) {
		aviso("constante redefinida: ");
		printf("%s, ",cnst);
		if(linea) printf(Lin_Msg,linea);
		else printf("\n");
	}
}
...
/* compila con los dos primeros par metros */
par6err=compila_par2(l,act_pro,ult_pro,codigo,lim1,lim2,255,coderr1,coderr2);
/* si hubo error al compilarlos, sale */
...
/****************************************************************************
	DEFWIN: compila condacto DEFWIN.
	  Entrada:      'l' puntero al puntero a la l¡nea con el par metro
			del condacto
			'act_pro' puntero al puntero a d¢nde dejar condacto
			compilado
...
err=compila_par6(l,act_pro,ult_pro,condacto,N_VENT-1,255,_E_WPNW,_E_NERR);
if(err) return(err);
